...
static ggml_tensor * build_moe_ffn(
    ...
    if (layer_idx < w.n_hash_layer && L.ffn_gate_tid2eid) {
        ...
        n_used = (int) routing.selected->ne[0];   // may be reduced (DFLASH_DS4_TOPK)
        ...
        // Intra-expert neuron sparsity probe (arXiv-style 50% magnitude cut):
        // zero activations below thr_mult * mean(|mid|) per (expert, token).
        // mean-threshold keeps ~43% for half-normal activations; quality probe
        // only — mul_mat_id still reads all rows, no byte savings here.
        static const float nmask_thr = [] {
            const char * e = std::getenv("DFLASH_DS4_NEURON_MEANMASK");
            return e ? (float) std::atof(e) : 0.0f;
        }();
        if (nmask_thr > 0.0f) {
