...
| `GGML_CUDA_GRAPH_MAX_KEYS` | 512 | Cap on captured CUDA/HIP graph executables kept per backend context, keyed by graph node address; least recently used entries are retired in batches. `0` disables the cap. |
